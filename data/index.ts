export const navItems = [
  { name: "About", link: "#about" },
  { name: "Testimonials", link: "#testimonials" },
  { name: "Contact", link: "#contact" },
];

export const gridItems = [
  {
    id: 1,
    title:
      "We manage your website, enhance SEO, host your emails and oversee social media to ensure your online presence thrives ",
    description: "",
    className:
      "lg:col-span-3 md:col-span-6 md:row-span-4 lg:min-h-[60vh] text-xs",
    imgClassName: "w-full h-full",
    titleClassName: "justify-end ",
    img: "/b1.svg",
    spareImg: "",
  },
  {
    id: 2,
    title: "Transforming your digital presence",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "",
    spareImg: "",
  },
  {
    id: 3,
    title: "With tech",
    description: "Constantly improving your business",
    className: "lg:col-span-2 md:col-span-3 md:row-span-2",
    imgClassName: "",
    titleClassName: "justify-center",
    img: "",
    spareImg: "",
  },
  {
    id: 4,
    title: "From R999/month",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-start",
    img: "/grid.svg",
    spareImg: "/b4.svg",
  },

  {
    id: 5,
    title: "Develop mobile apps for Android and iOS",
    description: "We also",
    className: "md:col-span-3 md:row-span-2",
    imgClassName: "absolute right-0 bottom-0 md:w-96 w-60",
    titleClassName: "justify-center md:justify-start lg:justify-center",
    img: "/b5.svg",
    spareImg: "/grid.svg",
  },
  {
    id: 6,
    title: "Do you want to work together?",
    description: "",
    className: "lg:col-span-2 md:col-span-3 md:row-span-1",
    imgClassName: "",
    titleClassName: "justify-center md:max-w-full max-w-60 text-center",
    img: "",
    spareImg: "",
  },
];

export const testimonials = [
  {
    quote:
      "Nexablend Digital revamped our website and took our SEO to the next level. Our online presence has never been stronger. Highly recommend!",
    name: "Jessica L.",
    title: "Marketing Director of The Lodge Rehab",
  },
  {
    quote:
      "The team at Nexablend Digital exceeded our expectations. From web design to social media management, their expertise is unmatched!",
    name: "Michael R.",
    title: "CEO of Tech Innovators Inc",
  },
  {
    quote:
      "Working with Nexablend Digital was the best decision we made. Our website looks amazing, and our social media engagement has skyrocketed!",
    name: "Sarah T.",
    title: "Founder of 4D Digital",
  },
  {
    quote:
      "Nexablend Digital’s attention to detail and commitment to our success is unparalleled. They truly care about helping our business grow.",
    name: "David M.",
    title: "Operations Manager at Fresh Start Fitness",
  },
  {
    quote:
      "From the moment we partnered with Nexablend Digital, we saw immediate improvements in our website traffic and customer interaction. They are true professionals!",
    name: "Willaim N.",
    title: "Founder of DTL Solutions",
  },
];

export const workExperience = [
  {
    id: 1,
    title: "Websites",
    desc: "We craft beautiful, engaging visual experiences that captivate the imagination of your users.",
    className: "md:col-span-2",
    thumbnail: "/exp1.svg",
  },
  {
    id: 2,
    title: "Mobile App Devopment",
    desc: "Designed and develop mobile app for both iOS & Android platforms.",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp2.svg",
  },
  {
    id: 3,
    title: "Marketing campaigns and strategies",
    desc: "From figuring out your target audience to utilizing design language to grab them, we've got you covered.",
    className: "md:col-span-2", // change to md:col-span-2
    thumbnail: "/exp3.svg",
  },
  {
    id: 4,
    title: "Social Media Marketing",
    desc: "We target all the big platforms on social media to bring you closer to your clients.",
    className: "md:col-span-2",
    thumbnail: "/exp4.svg",
  },
];

export const socialMedia = [
  {
    id: 1,
    img: "/facebook.svg",
    link: "https://www.facebook.com/nexablend.digital/",
  },
  {
    id: 2,
    img: "/instagram.svg",
    link: "https://www.instagram.com/nexablend.digital/",
  },
  {
    id: 3,
    img: "/whatsapp.svg",
    link: "https://wa.me/+27826008873",
  },
];
